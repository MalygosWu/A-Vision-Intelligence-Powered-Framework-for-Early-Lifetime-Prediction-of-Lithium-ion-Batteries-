...
class crossViT(nn.Module):
    def __init__(self, num_patches_small, num_patches_large, patch_dim_small, patch_dim_large, small_dim, large_dim,
                 num_classes = 1, small_depth = 1, large_depth = 4, cross_attn_depth = 1, multi_scale_enc_depth = 3,
                 heads = 3, pool = 'cls', dropout = 0, emb_dropout = 0, scale_dim = 4):
        super().__init__()
        
        
        self.to_patch_embedding_small = nn.Sequential(
            Rearrange('b n (k d2) -> b (n k) d2', d2=patch_dim_small),
            nn.Linear(patch_dim_small, small_dim),
        )
        self.to_patch_embedding_large = nn.Linear(patch_dim_large, large_dim)
    ...
    def forward(self, tokens):
        xs = self.to_patch_embedding_small(tokens)
        b, n, _ = xs.shape
        ...
        xl = self.to_patch_embedding_large(tokens)
        b, n, _ = xl.shape
